...
while 1:
    ...
    while welcomeScreen:
       for i in pygame.event.get():
           ...
           screen.blit(colorConfigFont.render("w-Eraser",True,(0,255,0)),(100,300))
    ...
    for i in pygame.event.get():
        if i.type==QUIT or pressed[K_q]:
            ...
            if pygame.mouse.get_pressed()==(1,0,0):
                pygame.draw.line(background,color,startPos,endPos,line_size)
            startPos=endPos
        if i.type == pygame.MOUSEBUTTONDOWN:
                if i.button == 1: # left click grows radius
                    radius = min(200, radius + 1)
                elif i.button == 3: # right click shrinks radius
